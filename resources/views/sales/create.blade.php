...
@section('header', 'New Sales Order')

@section('content')
<form method="POST" action="{{ route('sales.store') }}" class="space-y-6 max-w-5xl">
    @csrf
    <div class="bg-white rounded-xl shadow-sm p-6 grid grid-cols-2 gap-4">
        ...
        <div>
            ...
            <input type="date" name="order_date" value="{{ date('Y-m-d') }}" class="input" required>
        </div>
        <div>
        ...
        <table class="w-full text-sm">
            <thead class="bg-gray-50 text-gray-500 text-xs uppercase">
                <tr>
                    ...
                    <th class="px-4 py-3 text-right w-24">Stock</th>
            ...
            <tbody id="items-body">
                <tr id="empty-row" class="text-center text-gray-400">
                    <td colspan="6" class="py-8">Click "Add Item" to add products</td>
                </tr>
            </tbody>
    ...
    <div class="flex gap-3">
        ...
        <a href="{{ route('sales.index') }}" class="btn-ghost">Cancel</a>
    </div>
</form>

@push('scripts')
<script>
const products = @json($products->map(fn($p) => ['id'=>$p->id,'name'=>$p->name,'sku'=>$p->sku,'price'=>$p->selling_price,'stock'=>$p->totalStock()]));
let itemIndex = 0;
...
document.getElementById('add-item').addEventListener('click', () => {
    ...
    row.innerHTML = `
        <td class="px-4 py-2">
            <select name="items[${i}][product_id]" class="input product-select" required>
                ...
                ${products.map(p => `<option value="${p.id}" data-price="${p.price}" data-stock="${p.stock}">${p.name} (${p.sku})</option>`).join('')}
            </select>
        </td>
        <td class="px-4 py-2 text-right text-gray-500 stock-cell">—</td>
        ...
        <td class="px-4 py-2"><input type="number" name="items[${i}][unit_price]" class="input text-right price-input" min="0" step="0.01" value="0.00" required></td>
    ...
    row.querySelectorAll('.qty-input, .price-input').forEach(el => el.addEventListener('input', () => updateRow(row)));
