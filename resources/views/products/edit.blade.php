    ...
    <div class="bg-white rounded-xl shadow-sm p-6 space-y-4">
        ...
        <div class="grid grid-cols-3 gap-4">
            <div>
                ...
                <input type="number" name="cost_price" value="{{ old('cost_price', $product->cost_price) }}" step="0.01" min="0" class="input" required>
            </div>
            <div>
                <label class="label">Selling Price *</label>
                <input type="number" name="selling_price" value="{{ old('selling_price', $product->selling_price) }}" step="0.01" min="0" class="input" required>
            </div>
            <div>
                <label class="label">Minimum Stock *</label>
                <input type="number" name="minimum_stock" value="{{ old('minimum_stock', $product->minimum_stock) }}" step="0.01" min="0" class="input" required>
            </div>
        </div>
    </div>

    <div class="bg-white rounded-xl shadow-sm p-6 space-y-3">
        <h3 class="font-semibold text-gray-700 border-b pb-2">Options</h3>
        <label class="flex items-center gap-3 cursor-pointer">
            <input type="checkbox" name="track_batch"  value="1" @checked(old('track_batch', $product->track_batch))  class="w-4 h-4 rounded text-blue-600">
            <span class="text-sm text-gray-700">Track by batch/lot number</span>
        </label>
        <label class="flex items-center gap-3 cursor-pointer">
            <input type="checkbox" name="track_expiry" value="1" @checked(old('track_expiry', $product->track_expiry)) class="w-4 h-4 rounded text-blue-600">
            <span class="text-sm text-gray-700">Track expiry date</span>
        </label>
        <label class="flex items-center gap-3 cursor-pointer">
            <input type="checkbox" name="is_active"    value="1" @checked(old('is_active', $product->is_active)) class="w-4 h-4 rounded text-blue-600">
